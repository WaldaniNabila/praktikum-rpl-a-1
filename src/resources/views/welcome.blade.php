...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('build/assets/app-BkyXxD1_.css') }}">
...
<body class="bg-gray-50 text-gray-900 antialiased">
        ...
        <div class="max-w-7xl mx-auto grid grid-cols-2 sm:grid-cols-4 divide-x divide-gray-100">
            @foreach([
                ...
                <div class="py-5 text-center">
                    <div class="text-2xl font-semibold text-blue-700">{{ $num }}</div>
        ...
        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
            @foreach([
                ...
                <a href="#" class="flex flex-col gap-2 p-4 rounded-xl border border-gray-100 bg-gray-50 hover:border-blue-200 hover:bg-blue-50 transition-all group">
                    <div class="w-8 h-8 rounded-lg {{ $bg }}"></div>
                    <div>
                        <div class="text-sm font-medium text-gray-800 group-hover:text-blue-700 transition-colors">{{ $name }}</div>
                        <div class="text-xs text-gray-400 mt-0.5">{{ $count }} lowongan</div>
                    </div>
                </a>
            @endforeach
        </div>
    ...
    <section class="max-w-7xl mx-auto px-6 pb-10">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            ] as [$init, $bg, $color, $title, $company, $location, $salary, $type, $worktype])
                <div class="bg-white border border-gray-200 rounded-xl p-4 hover:border-blue-300 transition-all group">
                    <div class="flex gap-3 items-start mb-3">
                        <div class="w-10 h-10 rounded-lg {{ $bg }} {{ $color }} text-sm font-semibold flex items-center justify-center shrink-0">
                            {{ $init }}
                        </div>
                        <div class="flex-1 min-w-0">
                            <div class="text-sm font-medium text-gray-900 group-hover:text-blue-700 transition-colors truncate">{{ $title }}</div>
                    ...
                    <div class="flex flex-wrap gap-1.5 mb-2">
                        <span class="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-800 font-medium">{{ $type }}</span>
                        <span class="text-xs px-2 py-0.5 rounded-full bg-purple-50 text-purple-800 font-medium">{{ $worktype }}</span>
                    </div>
                    <div class="text-xs text-gray-400 mb-3">{{ $location }}</div>
                    <div class="flex items-center gap-2 border-t border-gray-100 pt-3">
                        <span class="text-xs font-semibold text-blue-700 flex-1">{{ $salary }}</span>
