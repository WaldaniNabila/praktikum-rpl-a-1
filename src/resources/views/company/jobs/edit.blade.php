        ...
        <div class="flex min-h-screen w-full overflow-hidden">
            ...
            <div class="flex-1 bg-slate-50 overflow-y-auto">
                <main class="p-8 lg:p-12 space-y-10 max-w-4xl mx-auto">
                    <div>
                        <h1 class="text-3xl font-extrabold text-slate-900 tracking-tight">Edit Lowongan Pekerjaan</h1>
                        <p class="text-slate-500 mt-2">Perbarui informasi lowongan: {{ $jobListing->title }}</p>
                    </div>
                    ...
                    <section class="rounded-2xl bg-white p-6 md:p-8 shadow-sm border border-slate-100/80">
                        <form action="{{ route('company.jobs.update', $jobListing->id) }}" method="POST" class="space-y-6">
                            @csrf
                            @method('PUT')
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="title" value="{{ old('title', $jobListing->title) }}" required class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                </div>
                                ...
                                <div>
                                    ...
                                    <input type="text" name="location" value="{{ old('location', $jobListing->location) }}" required class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Status Lowongan</label>
                                    <select name="is_open" class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                        <option value="1" {{ old('is_open', $jobListing->is_open) == true ? 'selected' : '' }}>Open (Buka)</option>
                                        <option value="0" {{ old('is_open', $jobListing->is_open) == false ? 'selected' : '' }}>Closed (Tutup)</option>
                                    </select>
                                </div>
                                ...
                                <div class="grid grid-cols-2 gap-4 col-span-1 md:col-span-2">
                                    ...
                                    <div>
                                        ...
                                        <select name="work_type" required class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                            <option value="on-site" {{ old('work_type', $jobListing->work_type) == 'on-site' ? 'selected' : '' }}>On-site</option>
                                            <option value="remote" {{ old('work_type', $jobListing->work_type) == 'remote' ? 'selected' : '' }}>Remote</option>
                                            <option value="hybrid" {{ old('work_type', $jobListing->work_type) == 'hybrid' ? 'selected' : '' }}>Hybrid</option>
                                        </select>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="number" name="salary_min" value="{{ old('salary_min', $jobListing->salary_min) }}" class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-xs font-bold text-slate-500 uppercase mb-2">Gaji Maksimum (Opsional)</label>
                                    <input type="number" name="salary_max" value="{{ old('salary_max', $jobListing->salary_max) }}" class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" rows="4" required class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">{{ old('description', $jobListing->description) }}</textarea>
                            </div>
                            ...
                            <div>
                                ...
                                <textarea name="requirements" rows="4" required class="w-full rounded-xl border-slate-200 text-sm focus:border-blue-500 focus:ring-blue-500 shadow-sm">{{ old('requirements', $jobListing->requirements) }}</textarea>
                            </div>
                            ...
                            <div class="flex justify-end gap-3 pt-6 border-t border-slate-100">
                                ...
                                <button type="submit" class="px-5 py-2.5 text-sm font-bold text-white bg-blue-600 rounded-xl hover:bg-blue-700 shadow-md shadow-blue-600/10 transition">Perbarui Lowongan</button>
                            </div>
                        </form>
                    </section>
                </main>
